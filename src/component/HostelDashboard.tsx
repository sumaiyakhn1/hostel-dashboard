...
export default function HostelDashboard() {
  ...
  const handleSubmit = async () => {
    ...
    const selectedRoom = availableRooms.find((r: any) => r.roomName === form.roomNo);
    if (!selectedRoom) return;

    const payload = {
      role: [], qualifications: [], workExperience: [],
      hostel: form.hostel,
    ...
    const confirmAssign = window.confirm(`Assign room ${form.roomNo} (${form.bedNo}) to ${student?.name || 'this student'}?`);
    if (!confirmAssign) return;

    setLoading(true);
    try {
      await hostelService.assignHostelRoom(payload);
      alert("Hostel Room assigned successfully!");
    } catch (error: any) {
  ...
  if (loading) return <div className="min-h-screen bg-[#e2e8f0] flex items-center justify-center font-bold text-slate-800 animate-pulse">Establishing Secure Connection...</div>;
  ...
  const getGlassClass = (value: string, isSpecial?: boolean) => {
    ...
    const state = value ? "bg-white/70 border-white" : "bg-white/20 border-white/60";
        ...
        <div className="md:w-1/4 p-10 flex flex-col justify-between border-r border-white/10">
          <div>
            <div className="flex items-center gap-2 mb-10 opacity-90">
              <div className="w-8 h-8 bg-slate-900 rounded-lg flex items-center justify-center text-white text-lg font-black">L</div>
              <span className="font-bold text-xl tracking-tight text-slate-900">Logo</span>
            </div>
          ...
          <div className="hidden md:block">
             <div className="px-3 py-1.5 bg-slate-800/5 rounded-full inline-block border border-slate-800/10">
               <p className="text-[8px] text-slate-400 font-black uppercase tracking-[0.2em]">Secure Student Portal</p>
             </div>
          </div>
        </div>
          ...
          <div className="w-full bg-white/30 backdrop-blur-md rounded-[2.5rem] border border-white/50 p-6 pt-12 relative">
            ...
            <div className="absolute top-[-35px] right-8">
              <div className="bg-white/80 backdrop-blur-2xl rounded-2xl shadow-lg border border-white px-4 py-2 flex items-center gap-4 min-w-[280px]">
                <div className="w-12 h-12 rounded-xl overflow-hidden bg-white shadow-inner flex-shrink-0 border border-slate-100 flex items-center justify-center relative">
                  ...
                  {fetchingStudent && <div className="absolute inset-0 bg-white/60 flex items-center justify-center animate-pulse"><div className="w-3 h-3 border border-red-600 border-t-transparent rounded-full animate-spin" /></div>}
                </div>
                <div>
                  <h3 className="font-bold text-slate-900 text-sm">{student?.name || "Welcome!"}</h3>
                  <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest">{student?.course || "Fill the form"}</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4 mt-6">
              ...
              <div className="space-y-1">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-2">Session</label>
                <select name="session" value={form.session} onChange={handleChange} className={getGlassClass(form.session)}>
                   <option value="">Select Session</option>
                   {student?.sessionList?.map((s: string) => <option key={s} value={s}>{s}</option>)}
                </select>
              </div>

              {/* Wing */}
              <div className="space-y-1">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-2">Wing</label>
                <select name="hostel" value={form.hostel} onChange={handleChange} className={getGlassClass(form.hostel)}>
              ...
              {/* Room Type */}
              <div className="space-y-1">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-2">Room Type</label>
                <select name="roomType" value={form.roomType} onChange={handleChange} className={getGlassClass(form.roomType)}>
              ...
              <div className="space-y-1">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-2 flex items-center gap-1">
                   Room No {fetchingRooms && <span className="w-1 h-1 bg-red-600 rounded-full animate-ping" />}
                </label>
                <select name="roomNo" value={form.roomNo} onChange={handleChange} className={getGlassClass(form.roomNo)} disabled={fetchingRooms}>
                  <option value="">{fetchingRooms ? "Locating..." : "Select Room No"}</option>
                  {Array.from(new Set(availableRooms.map((r: any) => r.roomName))).map((roomName: any) => (
              ...
              {/* Bed No */}
              <div className="space-y-1">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-2">Bed No</label>
                <select name="bedNo" value={form.bedNo} onChange={handleChange} className={getGlassClass(form.bedNo)}>
                  <option value="">Select Bed No</option>
                  {availableRooms.find(r => r.roomName === form.roomNo)?.beds?.map((b: any) => (
              ...
              <div className="space-y-1">
                <label className="text-[9px] font-black text-red-600/60 uppercase tracking-widest ml-2">Payment Freq</label>
                <select name="paymentFrequency" value={form.paymentFrequency} onChange={handleChange} className={getGlassClass(form.paymentFrequency, true)}>
              ...
              <div className="space-y-1">
                ...
                <input type="date" name="startDate" value={form.startDate} onChange={handleChange} className={getGlassClass(form.startDate)} />
              </div>
              <div className="space-y-1">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-2">End Date</label>
                <input type="date" name="endDate" value={form.endDate} onChange={handleChange} className={getGlassClass(form.endDate)} />
              </div>
